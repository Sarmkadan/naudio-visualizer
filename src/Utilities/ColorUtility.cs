...
namespace NAudioVisualizer.Utilities
{
    ...
    {
        /// <summary>
        /// The maximum value for a color channel (255).
        /// </summary>
        private const float ChannelMax = 255f;

        /// <summary>
        /// The number of degrees in a full color wheel (360).
        /// </summary>
        private const float HueDegrees = 360f;

        /// <summary>
        /// The number of degrees per hue sector (60).
        /// </summary>
        ...
        {
            float c = v * s;
            float x = c * (1 - MathF.Abs((h / HueSectorDegrees) % 2 - 1));
            float m = v - c;

            if (h < HueSectorDegrees)
            {
                r = c; g = x; b = 0;
            }
            else if (h < HueSectorDegrees * 2)
            {
                r = x; g = c; b = 0;
            }
            else if (h < HueSectorDegrees * 3)
            {
                r = 0; g = c; b = x;
            }
            else if (h < HueSectorDegrees * 4)
            {
                r = 0; g = x; b = c;
            }
            else if (h < HueSectorDegrees * 5)
            {
                r = x; g = 0; b = c;
            }
            else
        ...
        {
            int r = (int)Math.Clamp(color.R * factor, 0, (int)ChannelMax);
            int g = (int)Math.Clamp(color.G * factor, 0, (int)ChannelMax);
            int b = (int)Math.Clamp(color.B * factor, 0, (int)ChannelMax);
        ...
        {
            RgbToHsv(color.R / ChannelMax, color.G / ChannelMax, color.B / ChannelMax, out float h, out float s, out float v);
